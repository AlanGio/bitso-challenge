...
describe("<Grid />", () => {
  it("should render Grid component with 4 rows, 4 columns and islands empty", () => {
    ...
    expect(container.querySelectorAll(".cell-row").length).toBe(
      mockOnlySeaMap.length
    );

    // Check if has the right columns length
    expect(
      container.querySelectorAll(".cell-row:first-child > .cell").length
    ).toBe(mockOnlySeaMap[0].length);
